...
struct lefparser : boost::spirit::qi::grammar<Iterator, lef()>
{
  ...
  // shorthand for reduced verbosity in rule declarations
  template<typename Signature>
  struct Rule
  {
    typedef boost::spirit::qi::rule<Iterator, Signature> type;
  };
  ...
  typename Rule<SiteClass()>::type siteclass, padclass, coreclass, endcapclass;
  typename Rule<SiteSymmetry()>::type sitesym;
  typename Rule<SiteClass()>::type classrule;
  ...
  typename Rule<lef()>::type lef_file;
