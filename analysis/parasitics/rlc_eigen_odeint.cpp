...
{
   // just basically marks the connection between the inductor and the voltage,
   // because unlike capacitance, both are state variables.
   // Doing this brings in the V = LdI/dt equations:

   matrix(vnodeno, istateno) = 1;
...
struct rlc_tank {
  ...
  rlc_tank(double r, double l, double c, double vin) : r_(r), l_(l), c_(c), vin_(vin) {
    ...
    Matrix3f C = Matrix3f::Zero(), G = Matrix3f::Zero();

    stamp(G, 0, 1, 1.0/r_);
    stamp(C, 1, c_);
    // for the inductor, two operations:
    stamp(C, 2, l_);    // set derivative coefficient
    stamp_i(G, 1, 2);   // connect inductor current derivative to associated voltage
  ...
  void operator() (const state_type x, state_type& dxdt, double t) {
    ...
    for (std::size_t nodeno = 1; nodeno <= 2; ++nodeno)
    {
      // BOZO std::accumulate with lambda or something
      dxdt[nodeno] = 0.f;
      for (std::size_t stateno = 0; stateno <= 2; ++stateno)
      {
